...
Server::Server(std::uint16_t port) : acceptor_(ioc_) {
    ...
    if (!error) acceptor_.set_option(net::socket_base::reuse_address(true), error);
    if (!error) acceptor_.bind(endpoint, error);
    if (!error) acceptor_.listen(net::socket_base::max_listen_connections, error);
    if (error) {
...
Router& Server::router() noexcept { return router_; }
const Router& Server::router() const noexcept { return router_; }
...
std::uint16_t Server::port() const { return acceptor_.local_endpoint().port(); }
void Server::run() {
    while (acceptor_.is_open()) {
        beast::error_code error;
        tcp::socket socket{ioc_};
        acceptor_.accept(socket, error);
        if (error == net::error::operation_aborted || !acceptor_.is_open()) return;
        if (error) {
...
void Server::stop() noexcept { beast::error_code ignored; acceptor_.close(ignored); }
void Server::serve(tcp::socket socket) {
    ...
    for (;;) {
        ...
        spdlog::info("{} {} -> {}", request.method_string(), request.target(), response.result_int());
...
Response Server::dispatch(const Request& request) const {
    if (router_.matches(request)) return router_.dispatch(request);
    for (const auto& router : routers_) {
        if (router.matches(request)) return router.dispatch(request);
    }
    if (router_.matches_path(request)) return json_response(http::status::method_not_allowed, request, {{"error", "method_not_allowed"}});
    for (const auto& router : routers_) {
        if (router.matches_path(request)) return json_response(http::status::method_not_allowed, request, {{"error", "method_not_allowed"}});
    }
